...
class ModuleRetrieveAPIView(generics.RetrieveAPIView):
    ...
    def get_object(self):
        """Метод get_object вызывается для получения конкретного объекта модуля, который будет просмотрен.
        В этом методе выполняется проверка, оплачен ли модуль пользователем (module.is_paid_by(user)).
        Если модуль не был оплачен, вызывается исключение PermissionDenied с соответствующим сообщением.
        ...
        if not module.is_paid_by(user):  # Проверяем, оплатил ли пользователь модуль
            raise PermissionDenied(
                'You are not allowed to view this module in detail.')  # Вызываем исключение, если модуль не оплачен
        return module
...
class ModuleUpdateAPIView(generics.UpdateAPIView):
    ...
    def get_queryset(self):
        ...
        по полю user, который соответствует текущему пользователю (self.request.user)."""
